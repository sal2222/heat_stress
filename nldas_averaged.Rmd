...
heat_risk_days <-
  nldas_daily %>% 
    ...
    index_by(year = lubridate::year(date)) %>% 
      summarise(
        days_hi_g80 = sum(heat_index_max > 80),
        days_hi_g90 = sum(heat_index_max > 90),
        days_hi_g103 = sum(heat_index_max > 103),
        days_hi_g125 = sum(heat_index_max > 125),
...
heat_risk_days %>% 
  gather(key = "metric", value = "count", days_hi_g80:days_wbgt_gt90) %>%
  filter(metric == c("days_hi_g90", "days_wbgt_gt85")) %>%
...
heat_risk_hours <-
  nldas_tsibble %>% 
    ...
    index_by(year = lubridate::year(local_dttm)) %>% 
      summarise(
        hours_hi_g80 = sum(heat_index > 80),
        hours_hi_g90 = sum(heat_index > 90),
        hours_hi_g103 = sum(heat_index > 103),
        hours_hi_g125 = sum(heat_index > 125),
...
heat_risk_hours %>%
  filter(!year %in% "1989") %>% 
  gather(key = "metric", value = "count", hours_hi_g80:hours_wbgt_gt90) %>%
